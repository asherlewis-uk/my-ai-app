...
This document is the canonical repo-level reference for the iOS app as it exists today. It is written from the implementation in `ios/NestMind`, not from product intent alone.

If the Android repo is documented from this file later, keep the section order and subject intent the same.

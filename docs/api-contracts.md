...
This document covers the authenticated product API exposed by `supabase/functions/api/index.ts` and consumed by `ios/NestMind/Sources/Core/Networking/APIClient.swift`.
...
- the iOS client encodes request bodies in snake_case
- the backend currently accepts snake_case and camelCase for major write routes where helper readers are used
- responses are JSON with app-facing camelCase keys

Config source notes:

- the iOS request base is supplied by `SUPABASE_FUNCTIONS_BASE_URL` through the checked-in xcconfig and `Info.plist` chain
...
- `conversation` is the full rebuilt conversation payload
- `profile` is the latest profile snapshot
- `suggestedMemories` contains any persisted memory candidates returned by extraction
...
There is no checked-in `DELETE /v1/conversations/:id` handler in `supabase/functions/api/index.ts`.
